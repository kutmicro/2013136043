...
unsigned short V_CTRL = 0, V_SHIFT = 0, V_FN = 0;           // for multi-key input processing (동시 입력값)
unsigned short _ON_CTRL = 0, _ON_SHIFT = 0, _ON_FN = 0;     // for prevent unnecessory processing
unsigned short TM_CTRL = LD, TM_SHIFT = LD, TM_FN = LD;   // fading tmp value (각 키패드를 눌렀을 때 led값을 저장)
...
void READPIN() { //동시 입력을 할 때 사용되는 함수로서, 각 변수의 값을 읽어들인다.
  ...
  V_A = digitalRead(KP_CTRL);
  V_X = digitalRead(KP_SHIFT);
}
